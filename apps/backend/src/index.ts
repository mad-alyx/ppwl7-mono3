...
const app = new Elysia()
  // 1. Modifikasi CORS untuk Production (Vercel) & Lokal
  .use(
    cors({
  ...
  // 2. Keamanan: Cek API_KEY untuk akses data /users
  .onRequest(({ request, set }) => {
    ...
    if (url.pathname.startsWith("/users")) {
      ...
      // Izinkan jika datang dari Frontend resmi
      if (origin === frontendUrl) return;

      // Jika akses langsung browser/Postman, WAJIB ada ?key=
  ...
  .get("/auth/callback", async ({ query, set, cookie: { session }, redirect }) => {
    ...
    // Set cookie session dengan konfigurasi Production
    session.value = sessionId;
    session.maxAge = 60 * 60 * 24; // 1 hari
    session.path = "/";
    session.httpOnly = true;
    
    // Aktifkan Secure & SameSite None hanya di production (HTTPS)
    if (process.env.NODE_ENV === "production") {
      session.secure = true; 
      session.sameSite = "none";
    }

    // Redirect menggunakan ENV
...
// --- SERVER LISTEN (Hanya di Development) ---
